...
HTML, CSS, SCSS를 중심으로 화면 구조와 공통 스타일을 먼저 잡고, 필요한 JavaScript 기능은 AI를 참고해 구현하며 코드 흐름을 이해하고 수정하는 방식으로 작업했습니다.  
제가 주도한 부분과 AI 도움을 받은 부분을 과장 없이 구분해 기록하려고 했습니다.
...
- 복잡한 스크립트 구조와 기능 구현은 AI 도움을 받아 만들었고, 코드를 읽고 수정하면서 학습했습니다.
- 각 프로젝트 문서에는 직접 작업한 부분과 AI 도움을 받은 부분을 따로 정리했습니다.
...
| --- | --- | --- | --- |
| GUIDE-HTML | UI 컴포넌트를 정리하고 화면별 가이드 구조를 만든 퍼블리싱 학습 프로젝트 | 컴포넌트 마크업, SCSS 구조, 공통 스타일 정리, 일부 jQuery 동작은 AI 참고 | [보기](./GUIDE-HTML/) / [문서](./GUIDE-HTML/guide-html.md) |
| POTATO | 직접 그린 파츠 이미지를 조합해 이모티콘 PNG를 만드는 개인용 웹 앱 | 화면 구성, CSS 분리, UI 흐름 정리, IndexedDB/Canvas 기능은 AI 참고 | [보기](./POTATO/) / [문서](./POTATO/potato.md) |
| DailySummary | 업무 기록을 프로젝트별로 정리하고 JSON으로 관리하는 기록 도구 | 레이아웃, 입력 화면, 목록 UI 구성, JSON/필터 기능은 AI 참고 | [보기](./DailySummary/) / [문서](./DailySummary/daily-summary.md) |
| CharBox | 자주 사용하는 특수문자와 HTML 엔티티를 검색하고 복사하는 치트시트 | 표 구조, 검색 UI, 카테고리 UI 구성, 복사 기능은 AI 참고 | [보기](./CharBox/) / [문서](./CharBox/charbox.md) |
| SpellingDict | 헷갈리는 맞춤법을 카테고리별로 정리하고 초성 검색으로 찾는 개인용 사전 | 레이아웃, CSS 변수 체계, 카드/모달 UI 구성, 초성 검색/LocalStorage 기능은 AI 참고 | [보기](./SpellingDict/views/index.html) / [문서](./SpellingDict/spelling-dict.md) |
...
헷갈리는 맞춤법을 카테고리별로 정리하고, 초성 검색과 정렬로 빠르게 찾아볼 수 있는 개인용 사전입니다.

### 핵심 포인트

- 맞춤법 단어 쌍, 설명, 예시를 카드 형태로 구성
- 조사류, 어미류, 띄어쓰기, 혼동 단어, 외래어 표기 카테고리 정리
- 사이드바, 검색바, 정렬 셀렉트, 카드, 모달, 토스트 UI 구성
- 초성 검색, 검색어 하이라이트, 카테고리 필터, 가나다 정렬 기능은 AI 도움을 받아 구현
- 단어 추가/삭제 데이터와 테마 설정을 LocalStorage에 저장
...
| AI 도움을 받아 적용한 기능 | 검색, 초성 검색, 필터, 정렬, JSON 처리, LocalStorage, IndexedDB, Canvas, 이미지 저장 |
| 작업 방식 | 공통 구조와 스타일을 먼저 잡고, 스크립트는 필요한 기능 단위로 도움을 받아 구현 |

## AI 활용 기준

- 화면 구조, 마크업, CSS 정리, UI 배치와 문구는 직접 판단하고 수정했습니다.
- JavaScript 기능 중 검색, 필터, 저장, 파일 처리, IndexedDB, Canvas처럼 복잡한 부분은 AI를 참고했습니다.
- AI가 만든 코드는 화면에서 동작을 확인하고, 프로젝트 구조에 맞게 파일 위치와 클래스명, 문구를 수정했습니다.
- 면접이나 코드 설명을 위해 각 프로젝트 문서에 제가 직접 한 부분과 도움을 받은 부분을 구분해두었습니다.
